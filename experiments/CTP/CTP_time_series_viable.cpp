...
void runMCVIIncrements(
    ...
    const std::vector<std::pair<int64_t, std::vector<State>>>& completion_data,
  ...
  const auto [fsc, root] = planner.PlanAndEvaluate2(
      max_sim_depth, converge_thresh, 100000000000, max_time_ms, eval_depth,
      eval_epsilon, max_eval_steps, completion_data, completion_threshold,
      completion_reps);
}

std::vector<std::pair<int64_t, std::vector<State>>> runAOStarIncrements(
    CTP* pomdp, const BeliefDistribution& init_belief, std::mt19937_64& rng,
    int64_t eval_depth, int64_t eval_epsilon, int64_t max_time_ms,
  ...
  return RunAOStarAndEvaluate(
      root, 100000000000, max_time_ms, ptt, eval_depth, eval_epsilon,
      max_eval_steps, n_eval_trials, nb_particles_b0, eval_interval_ms,
      completion_threshold, completion_reps, rng, ptt, pomdp);
}
...
int main() {
  ...
  const int completion_reps = 100000;
  ...
  const auto aostarcomplete = runAOStarIncrements(
      aostar_ctp, init_belief, rng, eval_depth, eval_epsilon, max_time_ms,
      max_eval_steps, n_eval_trials, 10 * nb_particles_b0, eval_interval_ms,
  ...
  runMCVIIncrements(mcvi_ctp, init_belief, rng, max_sim_depth, max_node_size,
                    ...
                    max_eval_steps, aostarcomplete, completion_threshold,
                    completion_reps);
