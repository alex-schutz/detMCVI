...
void runMCVI(Mastermind* pomdp, const BeliefDistribution& init_belief,
...
void runAOStar(Mastermind* pomdp, const BeliefDistribution& init_belief,
...
void runPOMCP(Mastermind* pomdp, std::mt19937_64& rng, int64_t init_belief_size,
  ...
  POMCP::EvaluationWithGreedyTreePolicy(root_node, max_eval_steps,
                                        n_eval_trials, nb_particles_b0, pomdp,
                                        rng, solver, std::nullopt, "POMCP");
...
int main(int argc, char* argv[]) {
  ...
  // Run POMCP
  auto pomcp = new Mastermind(pomdp);
  ...
  // Run MCVI
  auto mcvi = new Mastermind(pomdp);
  ...
  // Compare to AO*
  auto aostar = new Mastermind(pomdp);
